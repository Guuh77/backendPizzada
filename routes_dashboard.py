...
async def obter_dashboard_evento(
    ...
    # Buscar estatísticas por sabor (agrupamento inteligente)
    sabores_query = """
        SELECT 
            sp.id as sabor_id,
        ...
        LEFT JOIN itens_pedido ip ON sp.id = ip.sabor_id
        LEFT JOIN pedidos p ON ip.pedido_id = p.id AND p.evento_id = :evento_id
        WHERE sp.ativo = 1
...
async def obter_oportunidades(
    ...
    for sabor in sabores_results:
        ...
        # Se tiver pedaços restantes, há oportunidade
        if pedacos_restantes > 0:
            ...
            if pedacos_para_completar <= 4:
                oportunidades.append({
                    ...
                    "tipo": "meia" if pedacos_restantes == 4 else "inteira"
                })
...
async def agrupar_pizzas_inteligente(
    ...
    # Buscar todos os sabores com pedidos
    query = """
        SELECT 
            sp.id as sabor_id,
            sp.nome as sabor_nome,
