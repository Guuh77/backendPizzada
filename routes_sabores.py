...
async def listar_sabores(
    ...
    query = "SELECT id, nome, preco_pedaco, ativo, data_cadastro FROM sabores_pizza"
...
async def obter_sabor(
    ...
    query = """
        SELECT id, nome, preco_pedaco, ativo, data_cadastro
        FROM sabores_pizza
        WHERE id = :sabor_id
    """
...
async def criar_sabor(
    ...
    # Inserir sabor
    insert_query = """
        INSERT INTO sabores_pizza (nome, preco_pedaco)
        VALUES (:nome, :preco)
    """
    ...
    with get_db_connection() as conn:
        ...
        cursor.execute(
            insert_query,
            {"nome": sabor.nome, "preco": sabor.preco_pedaco}
        )
        conn.commit()
