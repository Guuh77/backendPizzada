...
def authenticate_user(nome_completo: str, senha: str):
    """Autentica usuário"""
    query = """
        SELECT id, nome_completo, senha_hash, setor, is_admin, ativo, data_cadastro
        FROM usuarios
        WHERE nome_completo = :nome AND ativo = 1
    """
    
    result = execute_query(query, {"nome": nome_completo}, fetch_one=True)
